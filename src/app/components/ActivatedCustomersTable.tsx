...
interface ActivatedCustomersTableProps {
  customers: Customer[];
  formatCurrency: (value: number) => string;
}

export function ActivatedCustomersTable({ customers, formatCurrency }: ActivatedCustomersTableProps) {
    ...
    <div data-section="customer-list" className="space-y-4">
      <div className="flex items-center justify-between flex-wrap gap-3">
        <h3 className="text-sm sm:text-base font-semibold text-gray-900 dark:text-gray-100">
          Activated Customers ({customers.length})
        </h3>
        {totalPages > 1 && (
                ...
                <td className="px-4 py-3">
                  <div className="flex items-center gap-1.5">
                    ...
                    <span className="text-sm text-gray-600 dark:text-gray-400">
                      {new Date(customer.activationDate).toLocaleDateString('en-US', { month: 'short', day: 'numeric', year: 'numeric' })}
                    </span>
                  </div>
                </td>
                <td className="px-4 py-3">
